...
bool RoboticDevice::readInputFile()
{
	...
	if (auto feature = getFeature<dev::feature::DrawFeature>(dev::feature::FeatureType::Draw);
	    feature) {
		std::cout << feature->deviceName() << std::endl;
		if (auto fileHandler = feature->getHandler<dev::handler::FileHandler>(
		        dev::handler::HandlerType::File);
		    fileHandler) {
			int32_t fileId = fileHandler->openFile(
			    std::string(SOURCE_DIR) + "/InputFiles/input.txt", dev::handler::OpenMode::ToRead);
	...
	std::thread writer([this]() {
		if (auto feature = getFeature<dev::feature::DrawFeature>(dev::feature::FeatureType::Draw);
			...
			if (auto fileHandler = feature->getHandler<dev::handler::FileHandler>(
			        dev::handler::HandlerType::File);
			    fileHandler) {
				int32_t fileImageId = fileHandler->openFile(
				    std::string(SOURCE_DIR) + "/OutputFiles/output.bmp",
				    dev::handler::OpenMode::ToWrite);
				int32_t fileTextId = fileHandler->openFile(
				    std::string(SOURCE_DIR) + "/OutputFiles/output.txt",
				    dev::handler::OpenMode::ToWrite);

				if (auto roboticData = std::dynamic_pointer_cast<dev::data::RoboticDataStorage>(
				        m_dataStorage);
				    roboticData) {
					...
					fileHandler->getFileHandle(fileTextId)
					    .write(textResult.c_str(), textResult.size());
					fileHandler->getFileHandle(fileImageId)
...
void RoboticDevice::sendCommand(const std::string &command)
{
	if (auto feature = getFeature<dev::feature::DrawFeature>(dev::feature::FeatureType::Draw);
	    feature) {
		if (auto commandHandler = feature->getHandler<dev::handler::CommandHandler>(
		        dev::handler::HandlerType::Command);
		    commandHandler) {
			auto callBack = [this](const dev::command::AbstractCommandResult &result) {
				std::cout
				    << "Command completed with result: " << static_cast<uint16_t>(result.result())
				    << std::endl;
...
void RoboticDevice::attachDataStorage(const std::weak_ptr<AbstractDataStorage> &dataStorage)
{
	for (auto &feature : m_features) {
		...
		if (auto commandHandler = drawFeature->getHandler<dev::handler::CommandHandler>(
		        dev::handler::HandlerType::Command);
		    commandHandler) {
